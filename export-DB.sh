...
echo -e "\n$GREEN""$BOLD""Quick wait""$NC"
sleep 2s


docker-compose -f ./docker-compose.yml exec --privileged --user root embark_db mysqldump --all-databases --single-transaction --quick --lock-tables=false > full-backup-$(date +%F).sql -u root -p