...
install_emba() {
  echo -e "$GREEN""$BOLD""Installation of the firmware scanner emba""$NC"
...
reset_docker() {
  echo -e "$GREEN""$BOLD""Reset EMBArk docker images""$NC"

  docker images

  if docker images | grep -qE "^embark[[:space:]]*latest"; then
    echo -e "$GREEN""$BOLD""Found EMBArk docker environment - removing it""$NC"
  ...
  if docker images | grep -qE "^mysql[[:space:]]*latest"; then
    echo -e "$GREEN""$BOLD""Found mysql docker environment - removing it""$NC"
  ...
  if docker images | grep -qE "^redis[[:space:]]*5"; then
    echo -e "$GREEN""$BOLD""Found redis docker environment - removing it""$NC"
...
install_embark() {
  echo -e "$GREEN""$BOLD""Installation of the firmware scanning environment EMBArk""$NC"
  ...
  echo -e "$GREEN""$BOLD""Building EMBArk docker images""$NC"
  ...
  echo -e "$GREEN""$BOLD""Setup mysql and redis docker images""$NC"
  docker-compose up -d
  DU_RETURN=$?
  ...
  echo -e "$GREEN""$BOLD""Restarting EMBArk docker images""$NC"
