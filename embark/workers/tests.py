...
__copyright__ = (
    "Copyright 2021-2025 Siemens Energy AG, Copyright 2021-2025 The AMOS Projects"
)
...
class TestOrchestrator(TestCase):
    def setUp(self):
        ...
        user.save()
        test_config = Configuration.objects.create(
            user=user,
            name="test_config",
            ssh_user="test_user",
            ssh_password="test_password",
            ip_range="192.111.111.1/24",
        )  # nosec
        self.test_worker1 = Worker.objects.create(
            name="test_worker1",
            ip_address="192.111.111.1",
            system_info={},
            reachable=True,
        )  # pylint: disable=W0201
        self.test_worker2 = Worker.objects.create(
            name="test_worker2",
            ip_address="192.111.111.2",
            system_info={},
            reachable=True,
        )  # pylint: disable=W0201
    ...
    def test_orchestrator_add_worker(self):
        ...
        self.assertIn(
            self.test_worker1.ip_address, self.orchestrator.get_free_workers()
        )
    ...
    def test_orchestrator_remove_worker(self):
        ...
        self.assertNotIn(
            self.test_worker1.ip_address, self.orchestrator.get_busy_workers()
        )
    ...
    def test_orchestrator_assign_worker(self):
        ...
        self.assertEqual(
            self.orchestrator.get_busy_workers()[
                self.test_worker1.ip_address
            ].analysis_id,
            task.firmware_analysis_id,
        )
    ...
    def test_orchestrator_complex(self):
        ...
        self.orchestrator.remove_worker(self.test_worker1)
        self.assertIn(
            self.test_worker2.ip_address, self.orchestrator.get_busy_workers()
        )
        self.assertNotIn(
            self.test_worker1.ip_address, self.orchestrator.get_free_workers()
        )
        self.orchestrator.release_worker(self.test_worker2)
        self.assertIn(
            self.test_worker2.ip_address, self.orchestrator.get_free_workers()
        )
    ...
    def test_fifo_assign_task(self):
        ...
        self.assertEqual(
            orchestrator.get_busy_workers()[worker1.ip_address].analysis_id,
            task1.firmware_analysis_id,
        )
        self.assertEqual(
            orchestrator.get_busy_workers()[worker2.ip_address].analysis_id,
            task2.firmware_analysis_id,
        )
