...
def _run_script(dependency: DependencyType, update=True):
    """
    Runs script
    ...
    logger.info("Worker depenendencies setup started with script %s. Logs: %s", dependency.value, log_file)
    try:
...
def setup_dependencies(dependency: DependencyType, update=True):
    """
    Sets up the `dependency` dependencies required for offline workers

    :params dependency
    :params update: if false, aborts if files already present
    """
    if dependency == DependencyType.ALL:
        _run_script(DependencyType.DEPS, update)
        _run_script(DependencyType.REPO, update)
        _run_script(DependencyType.EXTERNAL, update)
        _run_script(DependencyType.DOCKERIMAGE, update)
    else:
        _run_script(dependency, update)
