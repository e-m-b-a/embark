...
def _update_dependency_version(worker: Worker, emba=None, nvd_head=None, epss_head=None, deb_list=None):
    """
...
def update_system_info(configuration: Configuration, worker: Worker):
    ...
    try:
        ...
        last_sync_nvd = exec_blocking_ssh(ssh_client, f"sudo bash -c 'cd {os.path.join(settings.WORKER_EMBA_ROOT, "external/nvd-json-data-feeds")} && git rev-parse HEAD'")
        last_sync_nvd = "N/A" if not re.match(commit_regex, last_sync_nvd) else last_sync_nvd
        last_sync_epss = exec_blocking_ssh(ssh_client, f"sudo bash -c 'cd {os.path.join(settings.WORKER_EMBA_ROOT, "external/EPSS-data")} && git rev-parse HEAD'")
        ...
        _update_dependency_version(worker, emba=emba_version, nvd_head=last_sync_nvd, epss_head=last_sync_epss, deb_list=deb_list)
