...
def _update_dependency_version(worker: Worker, emba=None, nvd_head=None, nvd_time=None, epss_head=None, epss_time=None, deb_list=None):  # pylint: disable=too-many-positional-arguments
    """
    Updates fields in workers dependency_version and checks if outdated
    :param worker: The worker to update
    :param emba: The emba version
    :param nvd_head: The nvd commit hash
    :param nvd_time: The nvd commit time
    :param epss_head: The epss commit hash
    :param epss_time: The epss commit time
    :param deb_list: The dependency list generated by `_parse_deb_list`
    """
    ...
    if emba is not None:
        worker.dependency_version.emba = emba
        worker.dependency_version.emba_outdated = version.emba != worker.dependency_version.emba

    if nvd_head is not None:
        worker.dependency_version.nvd_head = nvd_head

        if nvd_time is not None:
            worker.dependency_version.nvd_time = nvd_time

    if epss_head is not None:
        worker.dependency_version.epss_head = epss_head

        if epss_time is not None:
            worker.dependency_version.epss_time = epss_time

    if nvd_head is not None or epss_head is not None:
    ...
    if deb_list is not None:
        worker.dependency_version.deb_list = deb_list
        # Todo: Find oudated dependency
        worker.dependency_version.deb_outdated = True
...
def update_system_info(configuration: Configuration, worker: Worker):
    ...
    try:
        ...
        epss_head, epss_time = _fetch_external("external/EPSS-data")
        ...
        _update_dependency_version(worker, emba=emba_version, nvd_head=nvd_head, nvd_time=nvd_time, epss_head=epss_head, epss_time=epss_time, deb_list=deb_list)
