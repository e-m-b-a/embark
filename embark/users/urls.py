...
__author__ = (
    "Garima Chauhan, YulianaPoliakova, Benedikt Kuehne, VAISHNAVI UMESH, p4cx, m-1-k-3"
)
__license__ = "MIT"
...
urlpatterns = [
    ...
    path(
        "user/activate/<int:user_id>/<str:token>/",
        views.activate,
        name="embark-activate-user",
    ),
    ...
    path(
        "user/<int:user_id>/deactivate", views.deactivate, name="embark-deactivate-user"
    ),
    ...
    path(
        "log/<int:log_type>/<int:lines>/", views.get_log, name="log"
    ),  # TODO move to admin
]
