...
from embark.logreader import EMBA_P_PHASE, EMBA_S_PHASE, PROCESS_MAP, LogReader
...
class TestLogreader(TestCase):
    def setUp(self):
        ...
        self.test_file = os.path.join(settings.BASE_DIR.parent, "test/logreader/test-run.log")
        # check test_log file
        if not self.test_file.isfile():
            return logging.ERROR

    def test_in_pro(self):
        logr = LogReader(self.analysis_id)
        global PROCESS_MAP
        with open(self.test_file, 'r', encoding='UTF-8') as test_file:
            for line in test_file:
                ...
                self.assertTrue(filecmp(f"{settings.EMBA_LOG_ROOT}/{self.analysis_id}/logreader.log", f"{settings.EMBA_LOG_ROOT}/{self.analysis_id}/emba_logs/emba.log"), "Files not equal?!")   # check correctness of regexes
                ...
                _, phase_identifier = logr.phase_identify()
