...
class Firmware(models.Model):
    ...
    firmware_Architecture = models.TextField(help_text='Architecture of the linux firmware [MIPS, ARM, x86, x64, PPC]
    ,-a will be added')
    cwe_checker = models.BooleanField(help_text='Enables cwe-checker,-c will be added',
    default=False)
    docker_container = models.BooleanField(help_text='Run emba in docker container, -D will be added', default=False)
    deep_extraction = models.BooleanField(help_text='Enable deep extraction - try to extract every file two times with binwalk, -x will be added',
    default=False)
    log_path = models.BooleanField(help_text='Ignores log path check, -i will be added', default=False)
    grep_able_log = models.BooleanField(help_text='Create grep-able log file in [log_path]/fw_grep.log, -g will be added',
    default=False)
    relative_paths = models.BooleanField(help_text='Prints only relative paths, -s will be added',
    default=False)
